...
def softmax(a):
    ...
    """
    sn = np.exp(a)
    sd = np.sum(np.exp(a), axis=1).reshape(-1, 1)
    return sn/sd
...
class Network:
    ...
    def train(self, minibatch):
        ...
        self.weights=self.weights-(self.hyperparameters.learning_rate*1/y.shape[0])*X.T@(y_hat-y)
        if self.out_dim>1:
            y_pred=data.onehot_decode(y_hat)
            y=data.onehot_decode(y)
        else:
            y_pred = np.where(y_hat > 0.5, 1, 0)
        return np.sum(loss), np.sum(y_pred == y)/y.shape[0]
